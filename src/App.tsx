...
export default function App() {
  ...
  const [searchSource, setSearchSource] = useState<"soundcloud" | "spotify">(
    "soundcloud",
  );
  ...
  const importSpotifyPlaylist = async () => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/api/spotify/playlist?url=${encodeURIComponent(spotifyPlaylistUrl)}`,
      );
      ...
      for (let i = 0; i < data.tracks.length; i++) {
        ...
        try {
          ...
          const ytRes = await fetch(
            `${API_BASE_URL}/api/search/youtube?query=${encodeURIComponent(searchQuery)}`,
          );
          ...
          if (ytData && ytData.items && ytData.items.length > 0) {
            ...
            importedTracks.push({
              ...
              artwork_url:
                track.thumbnail ||
                firstResult.thumbnail?.thumbnails?.[0]?.url ||
                "",
              thumbnail:
                track.thumbnail ||
                firstResult.thumbnail?.thumbnails?.[0]?.url ||
          ...
          console.error(
            `Gagal mencari lagu YouTube untuk: ${track.title}`,
            err,
          );
        }
      }
      ...
      alert(
        "Gagal mengimpor playlist Spotify. Pastikan link benar dan playlist publik.",
      );
    } finally {
  ...
  const [sharedPlaylistData, setSharedPlaylistData] = useState<Playlist | null>(
    null,
  );
        ...
        if (sharedData) {
          ...
          const playlist: Playlist = {
            ...
            tracks: (decoded.t || []).map((t: any) => ({
              permalink_url: t.p,
              title:
                t.t || t.p.split("/").pop()?.replace(/-/g, " ") || "Untitled",
      ...
      if (isYouTube) {
        ...
        if (data.status && data.data) {
          metadata = {
            ...
            permalink_url: permalink_url,
            thumbnail:
              data.data.thumbnail || data.data.artwork_url || data.data.image,
          };
        }
      }
  ...
  const fetchHistory = async () => {
    ...
    try {
      if (Capacitor.isNativePlatform()) {
        const savedHistory: any = await localforage.getItem(
          "soundstream_history",
        );
    ...
    try {
      ...
      setHistory((prev) => {
        const filtered = prev.filter(
          (t) => t.permalink_url !== track.permalink_url,
        );
  ...
  const initAudioContext = () => {
    if (!audioCtxRef.current && audioRef.current) {
      try {
        ...
        const isStreamingYT =
          currentTrack?.url?.includes("ytcontent.com") ||
        ...
        if (isStreamingYT) {
          console.log(
            "Streaming YouTube terdeteksi: Melewati AudioContext untuk menghindari CORS error.",
          );
          return;
        }
    ...
    const updatedPlaylists = playlists.map((p) => {
      if (p.id === playlistToEdit.id) {
        return {
          ...
          artwork_url: editPlaylistImage.trim(),
        };
      }
      return p;
    });
  ...
  const removeFromPlaylist = (playlistId: string, trackUrl: string) => {
    const updated = playlists.map((p) => {
      if (p.id === playlistId)
        return {
          ...p,
          tracks: p.tracks.filter((t) => t.permalink_url !== trackUrl),
        };
      return p;
    });
    setPlaylists(updated);
    if (activePlaylist && activePlaylist.id === playlistId) {
      setActivePlaylist({
        ...activePlaylist,
        tracks: activePlaylist.tracks.filter(
          (t) => t.permalink_url !== trackUrl,
        ),
      });
    }
  };
  ...
  const deletePlaylist = (playlistId: string) => {
    setConfirmModal({
      ...
      message:
        "Playlist ini akan dihapus permanen dan tidak bisa dikembalikan.",
      onConfirm: () => {
  ...
  const handleSearch = async (e?: React.FormEvent, forceQuery?: string) => {
    ...
    try {
      if (searchSource === "soundcloud") {
        const res = await fetch(
          `https://api.siputzx.my.id/api/s/soundcloud?query=${encodeURIComponent(searchQuery)}`,
        );
        const data = await res.json();
        if (data.status && data.data) {
        ...
        if (!forceQuery) {
          const metaRes = await fetch(
            `${API_BASE_URL}/api/search/metadata?query=${encodeURIComponent(searchQuery)}`,
          );
  ...
  const fetchLyrics = async (title: string, artist?: string) => {
    ...
    try {
      ...
      const cleanTitle = title
        .replace(/\(.*\)|\[.*\]/g, "")
        .replace(/original|cover|remix|official|lyric|video/gi, "")
        .trim();
  ...
  const playTrack = async (permalink_url: string, trackData?: any) => {
    ...
    if (!isOnline) {
      const offlineData: any = await localforage.getItem(
        `track_${permalink_url}`,
      );
      if (!offlineData) {
    ...
    try {
      ...
      if (
        permalink_url.startsWith("spotify_jit_") ||
        (trackData && trackData.isSpotify)
      ) {
        ...
        const ytRes = await fetch(
          `${API_BASE_URL}/api/search/youtube?query=${encodeURIComponent(query)}`,
        );
        const ytData = await ytRes.json();
      ...
      // 2. Gunakan backend (proxy) untuk menghindari CORS saat mengambil audio dari YouTube
      const isYouTube =
        finalPermalinkUrl.includes("youtube.com") ||
        finalPermalinkUrl.includes("youtu.be");
        ...
        if (data.status && data.data) {
          trackInfo = {
            ...
            user:
              metadataToUse?.user ||
              data.data.user ||
              getArtistFromUrl(finalPermalinkUrl),
            thumbnail:
              metadataToUse?.artwork_url ||
              metadataToUse?.thumbnail ||
              data.data.thumbnail,
      ...
      if (trackInfo && trackInfo.url) {
        ...
        if (!audioRes.ok) {
          throw new Error(
            `Gagal mengunduh file (Status: ${audioRes.status}). Server audio mungkin memblokir akses.`,
          );
        }
    ...
    setIsShuffle((prev) => {
      ...
      if (newShuffle && queue.length > 0) {
        ...
        const originalIndex = queue.findIndex(
          (t) => t.permalink_url === currentTrack.permalink_url,
        );
          ...
          artwork: [
            {
              src:
                ...
                "https://picsum.photos/seed/music/512/512",
              sizes: "512x512",
              type: "image/png",
            },
          ],
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
                    {trendingResults.slice(0, 5).map((track, idx) => (
                      ...
                      >
                        ...
                        <div>
                          <h3 className="font-semibold text-zinc-100 truncate text-sm sm:text-base capitalize">
                            {track.title || track.permalink.replace(/-/g, " ")}
                          </h3>
                          <p className="text-xs text-zinc-400 truncate">
                            {getArtistFromUrl(track.permalink_url)}
                          </p>
                        </div>
                      </motion.div>
                    ))}
                ...
                <div className="space-y-6">
                  <div className="flex items-center justify-between">
                    <h3 className="text-xl font-bold text-zinc-400">
                      Versi mana yang kamu cari?
                    </h3>
                    <button
                      onClick={() => setIsSelectingArtist(false)}
                      className="text-sm text-emerald-500 font-bold"
                    >
                      Lewati
                    </button>
                  </div>
                    ...
                    {metadataSuggestions.map((item, idx) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-[10px] text-emerald-500 uppercase font-bold tracking-widest mt-1 opacity-60">
                            {item.album}
                          </p>
                        </div>
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="p-2 bg-emerald-500 text-zinc-950 rounded-full shadow-lg active:scale-90 transition-transform"
                >
                        ...
                        <div className="flex items-start gap-6">
                          <div className="relative w-32 h-32 sm:w-48 sm:h-48 rounded-3xl overflow-hidden shadow-2xl bg-zinc-900 flex-shrink-0 group">
                            {activePlaylist.artwork_url ? (
                              <img
                                src={activePlaylist.artwork_url}
                                alt=""
                                className="w-full h-full object-cover"
                              />
                              ...
                              <img
                                src={
                                  activePlaylist.tracks[0].artwork_url ||
                            ...
                            <button
                              onClick={(e) =>
                                openEditPlaylistModal(e, activePlaylist)
                              }
                            ...
                            <div className="flex flex-wrap items-center gap-3 sm:gap-4">
                              ...
                              <div className="flex items-center gap-2 sm:gap-3">
                                <button
                                  ...
                                  className="p-2.5 sm:p-3 bg-zinc-900 rounded-full hover:bg-zinc-800 transition-colors text-emerald-500"
                                >
                                ...
                                <button
                                  ...
                                  className="p-2.5 sm:p-3 bg-zinc-900 rounded-full hover:bg-red-500/20 hover:text-red-500 transition-colors"
                                >
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                            {playlists.map((p) => (
                              <div
                                ...
                                className="flex flex-col p-4 bg-zinc-900/40 rounded-[2rem] hover:bg-zinc-900 transition-all cursor-pointer group shadow-lg border border-transparent hover:border-zinc-800"
                              >
                                <div className="relative aspect-square mb-4 rounded-2xl overflow-hidden bg-zinc-800 shadow-inner">
                                  ...
                                  <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                    <div className="w-12 h-12 bg-emerald-500 rounded-full flex items-center justify-center text-zinc-950 shadow-xl">
                                      <Play className="w-6 h-6 fill-current ml-1" />
                                    </div>
                                  </div>
                                </div>
                                <h4 className="font-bold text-lg truncate px-1">
                                  {p.name}
                                </h4>
                                <p className="text-zinc-500 text-sm px-1">
                                  {p.tracks.length} lagu
                                </p>
                              </div>
                            ))}
                          </div>
                  ...
                  <div>
                    {downloadedTracks.length === 0 ? (
                      ...
                      <TrackList
                        ...
                        openAddModal={openAddModal}
                        formatTime={formatTime}
                        getArtist={(url: any) => "Downloaded Track"}
                        setConfirmModal={setConfirmModal}
                      />
                    )}
                  </div>
                      ...
                      <div className="space-y-4">
                        <div className="flex justify-end">
                          <button
                            ...
                            className="text-sm text-zinc-500 hover:text-red-500 flex items-center gap-1"
                          >
            ...
            <div className="relative flex-1 flex flex-col p-8 sm:p-12 overflow-y-auto no-scrollbar">
              <div className="flex justify-between items-center mb-10">
                <button
                  ...
                  className="p-2 bg-white/5 rounded-full backdrop-blur-md"
                >
                ...
                <div className="text-center">
                  ...
                  <p className="text-sm text-zinc-400 truncate max-w-[200px]">
                    {activePlaylist?.name || "SoundStream"}
                  </p>
                </div>
                <button
                  className="p-2 bg-white/5 rounded-full backdrop-blur-md"
              ...
              <div className="w-full max-w-xl mx-auto space-y-8 mt-12">
                ...
                <div className="flex items-center justify-between">
                  <button
                    ...
                    className={`p-2 transition-colors ${isShuffle ? "text-emerald-500" : "text-zinc-600"}`}
                  >
                  ...
                  <button
                    ...
                    className={`p-2 transition-colors ${repeatMode !== "off" ? "text-emerald-500" : "text-zinc-600"}`}
                  >
                    {repeatMode === "one" ? (
                ...
                <div className="flex items-center justify-around pt-4">
                  <button
                    onClick={() => {
                      if (!lyrics) fetchLyrics(currentTrack.title);
                      setIsLyricsOpen(true);
                    }}
                    className={`flex flex-col items-center gap-1 transition-colors ${isLyricsOpen ? "text-emerald-500" : "text-zinc-500"}`}
                  >
                  ...
                  >
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-widest">
                      {isTrackInAnyPlaylist ? "Tersimpan" : "Tambah"}
                    </span>
                  </button>
                  ...
                  >
                    <Zap
                      className={`w-6 h-6 ${isBassBoost ? "fill-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.5)]" : ""}`}
                    />
                    <span className="text-[10px] font-bold uppercase tracking-widest">
                      Boost
                    </span>
                  ...
                  <button
                    onClick={(e) => downloadTrack(e, currentTrack)}
                    className="flex flex-col items-center gap-1 text-zinc-500"
                  >
                    {downloadingTracks.has(currentTrack.permalink_url!) ? (
                      <Loader2 className="w-6 h-6 animate-spin" />
                    ) : (
                ...
                >
                  ...
                  <div className="flex-1 overflow-y-auto no-scrollbar pb-20">
                    {isLoadingLyrics ? (
                      ...
                      <p className="text-3xl sm:text-4xl font-bold leading-tight whitespace-pre-wrap text-zinc-200">
                        {lyrics}
                      </p>
                    ) : (
            ...
            <div className="text-center space-y-4 max-w-xs">
              ...
              <div className="space-y-2">
                <p className="text-emerald-400 text-sm font-medium animate-pulse">
                  Sedang mengunduh file audio ke memori sementara...
                </p>
                <p className="text-zinc-500 text-xs leading-relaxed">
                  Lagu dengan durasi lama (10+ menit) mungkin membutuhkan waktu
                  pemrosesan lebih. Mohon tunggu sebentar.
                </p>
              </div>
            </div>
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[70] flex items-center justify-center p-6">
          <motion.div
            ...
            className="bg-zinc-900 rounded-[2.5rem] p-8 w-full max-w-md shadow-2xl border border-zinc-800"
          >
            <div className="flex items-center justify-between mb-8">
              <h3 className="text-2xl font-bold">
                {trackToAdd ? "Tambah ke Playlist" : "Buat Playlist Baru"}
              </h3>
              <button
                onClick={() => {
                  setIsAddModalOpen(false);
            ...
            {trackToAdd && (
              <div className="space-y-3 max-h-60 overflow-y-auto no-scrollbar pr-2 mb-8">
                {playlists.map((p) => {
                  const isTrackInThisPlaylist = p.tracks.some(
                    (t) => t.permalink_url === trackToAdd.permalink_url,
                  );
                  return (
                    <button
                      key={p.id}
                      onClick={() => addToPlaylist(p.id, trackToAdd)}
                      className="w-full flex items-center gap-4 p-4 bg-zinc-800/50 hover:bg-zinc-800 rounded-2xl transition-all"
                    >
                      <div className="w-10 h-10 bg-zinc-700 rounded-lg flex items-center justify-center">
                        {isTrackInThisPlaylist ? (
                      ...
                      <span
                        className={`font-bold ${isTrackInThisPlaylist ? "text-emerald-500" : "text-white"}`}
                      >
                        {p.name}
                      </span>
                    </button>
            ...
            <form onSubmit={createPlaylist} className="flex gap-2">
              ...
              <button
                type="submit"
                className="bg-emerald-500 text-zinc-950 p-3 rounded-xl font-bold active:scale-95 transition-transform"
              >
                <Plus className="w-6 h-6" />
              </button>
            </form>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[70] flex items-center justify-center p-6">
          ...
          >
            <div className="flex items-center justify-between mb-8">
              ...
              <button
                onClick={() => setIsEditPlaylistModalOpen(false)}
                className="p-2 bg-zinc-800 rounded-full"
              >
                <X className="w-5 h-5" />
              </button>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[70] flex items-center justify-center p-6">
          ...
          >
            <div className="flex items-center justify-between mb-8">
              ...
              {!isImportingSpotify && (
                <button
                  onClick={() => setIsSpotifyImportModalOpen(false)}
            ...
            {isImportingSpotify ? (
              ...
              <div className="space-y-6">
                <p className="text-zinc-400 text-sm leading-relaxed">
                  Tempelkan link playlist Spotify publik Anda di bawah. Kami
                  akan mencari versi YouTube-nya secara otomatis.
                </p>
                <div className="space-y-4">
                  <input
                    type="text"
          ...
          >
            <div className="text-center space-y-4">
              ...
              <p className="text-zinc-400">
                Seseorang membagikan playlist <b>"{sharedPlaylistData.name}"</b>{" "}
                dengan {sharedPlaylistData.tracks.length} lagu kepada Anda.
              </p>
...
function NavButton({
  ...
  return (
    ...
    >
      <motion.div animate={{ scale: active && !disabled ? 1.2 : 1 }}>
        {icon}
      </motion.div>
      <span className="text-[10px] font-bold uppercase tracking-widest">
        {label}
  ...
  <div className="space-y-2">
    {tracks.map((track: any, idx: number) => (
      ...
      >
        <div className="relative w-14 h-14 rounded-xl overflow-hidden bg-zinc-900 flex-shrink-0 shadow-md">
          <img
            src={
              track.artwork_url ||
              track.thumbnail ||
              "https://picsum.photos/seed/music/100/100"
            }
            alt=""
            className="w-full h-full object-cover"
          />
        ...
        <div className="flex-1 min-w-0">
          <h4 className="font-bold text-zinc-100 truncate capitalize">
            {track.title || track.permalink.replace(/-/g, " ")}
          </h4>
          <p className="text-xs text-zinc-500 truncate">
            {track.user || getArtist(track.permalink_url)}
          </p>
        </div>
        <div
          ...
          onClick={(e) => e.stopPropagation()}
        >
          {downloadedTracks.some(
            (t: any) => t.permalink_url === track.permalink_url,
          ) ? (
            <button
              onClick={(e) => {
                setConfirmModal({
          ...
          {showRemove ? (
            <button
              onClick={() => {
                setConfirmModal({
                  ...
                  onConfirm: () =>
                    removeFromPlaylist(playlistId, track.permalink_url),
                });
              }}
            ...
            <button
              onClick={(e) => openAddModal(e, track)}
              className="p-2 text-zinc-700 hover:text-emerald-500"
            >
