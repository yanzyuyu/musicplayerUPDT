...
export default function App() {
  ...
  const fetchTrending = async () => {
    ...
    try {
      // Searching for "Top Hits" or "Trending" to get initial recommendations
      const res = await fetch(`https://api.siputzx.my.id/api/s/soundcloud?query=top%20hits%202026`);
      const data = await res.json();
      if (data.status && data.data) {
        setTrendingResults(data.data.slice(0, 15)); // Get top 15 trending
      }
    } catch (error) {
  ...
  const TrackList = ({ tracks, showRemove = false, playlistId = null }: { tracks: any[], showRemove?: boolean, playlistId?: string | null }) => (
    <div className="grid gap-3">
      {tracks.map((result, idx) => {
        ...
        return (
        <div key={idx} className="flex items-start sm:items-center gap-1 sm:gap-2 group bg-zinc-900/30 hover:bg-zinc-800/40 active:scale-[0.98] transition-all rounded-xl border border-transparent hover:border-zinc-800/50 shadow-sm">
          <button 
            onClick={() => {
              setQueue(tracks);
              if (isShuffle) {
    ...
    <div className="h-screen bg-zinc-950 text-zinc-50 flex flex-col font-sans overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex bg-zinc-800/50 rounded-lg p-1 overflow-x-auto no-scrollbar">
              <button 
                ...
                className={`px-3 sm:px-4 py-1.5 rounded-md text-xs sm:text-sm font-medium transition-all active:scale-95 whitespace-nowrap ${activeTab === 'search' ? 'bg-zinc-700 text-white shadow-lg' : 'text-zinc-400 hover:text-zinc-200'}`}
              >
                Search
              </button>
              <button 
                onClick={() => setActiveTab('playlists')}
                className={`px-3 sm:px-4 py-1.5 rounded-md text-xs sm:text-sm font-medium transition-all active:scale-95 whitespace-nowrap ${activeTab === 'playlists' ? 'bg-zinc-700 text-white shadow-lg' : 'text-zinc-400 hover:text-zinc-200'}`}
              >
                Playlists
              </button>
              <button 
                onClick={() => { setActiveTab('history'); setActivePlaylist(null); }}
                className={`px-3 sm:px-4 py-1.5 rounded-md text-xs sm:text-sm font-medium transition-all active:scale-95 whitespace-nowrap ${activeTab === 'history' ? 'bg-zinc-700 text-white shadow-lg' : 'text-zinc-400 hover:text-zinc-200'}`}
              >
                History
              </button>
              <button 
                onClick={() => { setActiveTab('downloads'); setActivePlaylist(null); }}
                className={`px-3 sm:px-4 py-1.5 rounded-md text-xs sm:text-sm font-medium transition-all active:scale-95 whitespace-nowrap ${activeTab === 'downloads' ? 'bg-zinc-700 text-white shadow-lg' : 'text-zinc-400 hover:text-zinc-200'}`}
              >
                Downloads
              </button>
            </div>
          ...
          {activeTab === 'search' && (
            <form onSubmit={handleSearch} className="flex-1 w-full relative" role="search" aria-label="Search music">
              ...
              <button 
                ...
                className="absolute right-1.5 sm:right-2 top-1/2 -translate-y-1/2 bg-emerald-500 hover:bg-emerald-400 text-zinc-950 p-1.5 sm:p-2 rounded-full transition-all active:scale-90 focus:outline-none focus:ring-2 focus:ring-emerald-500"
      ...
      <main className="flex-1 w-full max-w-4xl mx-auto px-4 sm:px-6 py-6 overflow-y-auto no-scrollbar relative pb-32">
        {isLyricsOpen && currentTrack ? (
          <div className="fixed inset-0 bg-zinc-950 z-[60] overflow-y-auto flex flex-col animate-in fade-in slide-in-from-bottom duration-300">
            {/* Lyrics Background Gradient */}
              ...
              <div className="flex justify-between items-start mb-12 sticky top-0 py-4 z-10">
                <div className="flex gap-4 items-center">
                  <img 
                    ...
                    alt="" 
                    className="w-12 h-12 sm:w-16 sm:h-16 rounded-lg shadow-2xl object-cover"
                  />
                  <div>
                ...
                <button 
                  ...
                  className="p-2 bg-white/10 hover:bg-white/20 active:scale-90 rounded-full transition-all backdrop-blur-md"
                >
              ...
              <div className="flex-1 flex flex-col items-center justify-start max-w-3xl mx-auto w-full">
                {isLoadingLyrics ? (
                  <div className="flex flex-col items-center gap-4 text-zinc-400 my-auto">
                    ...
                    <p className="text-lg font-medium">Fetching lyrics...</p>
                  </div>
                ) : lyrics ? (
                  <div className="w-full pb-32">
                    <p className="whitespace-pre-wrap text-2xl sm:text-4xl md:text-5xl leading-tight sm:leading-tight text-zinc-300 font-bold text-center sm:text-left selection:bg-emerald-500/30">
                      {lyrics}
                    </p>
                  </div>
                ) : (
                  <div className="my-auto text-zinc-500 text-center">
                    <Mic2 className="w-20 h-20 mx-auto mb-6 opacity-10" />
                    <p className="text-xl font-medium">Instruments only, no lyrics found.</p>
                  </div>
                )}
              </div>
            </div>
            ...
            {results.length > 0 ? (
              <div className="space-y-4 animate-in fade-in duration-500">
                <h2 className="text-xl font-semibold mb-6 flex items-center gap-2">
                  <Search className="w-5 h-5 text-emerald-400" /> Search Results
                </h2>
                <TrackList tracks={results} />
              </div>
            ) : query.trim() ? (
              <div className="flex flex-col items-center justify-center h-64 text-zinc-500">
                {isSearching ? <Loader2 className="w-12 h-12 animate-spin text-emerald-500" /> : <p>No results found for "{query}"</p>}
              </div>
            ) : (
              <div className="space-y-8 animate-in fade-in duration-700">
                <div className="flex items-center justify-between">
                  <h2 className="text-xl font-bold flex items-center gap-2">
                    <Flame className="w-6 h-6 text-orange-500 fill-orange-500" /> Trending Today
                  </h2>
                </div>
                {isLoadingTrending ? (
                  <div className="grid gap-3">
                  ...
                  <TrackList tracks={trendingResults} />
                )}
              </div>
            )}
          </>
        ...
        {activeTab === 'playlists' && !activePlaylist && (
          <div className="space-y-6">
            ...
            <form onSubmit={createPlaylist} className="flex gap-2">
              ...
              />
              <button type="submit" className="bg-emerald-500 hover:bg-emerald-400 text-zinc-950 px-4 py-2 rounded-lg font-medium transition-all active:scale-95 flex items-center gap-2 shadow-lg">
                ...
                playlists.map(playlist => (
                  <div key={playlist.id} className="bg-zinc-800/30 border border-zinc-700/50 rounded-xl p-4 flex items-center justify-between group hover:bg-zinc-800/50 active:scale-[0.98] transition-all cursor-pointer shadow-md" onClick={() => setActivePlaylist(playlist)}>
                    ...
                    <button 
                      ...
                      className="p-2 text-zinc-500 hover:text-red-400 opacity-0 group-hover:opacity-100 active:scale-75 transition-all focus:opacity-100 focus:outline-none"
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-4">
              <button 
                ...
                className="p-2 hover:bg-zinc-800 active:scale-75 rounded-full transition-all"
            ...
            {activePlaylist.tracks.length === 0 ? (
              ...
              <TrackList tracks={activePlaylist.tracks} showRemove={true} playlistId={activePlaylist.id} />
            )}
          </div>
        )}
            ...
            <div className="flex items-center justify-between">
              ...
              {history.length > 0 && (
                <button 
                  ...
                  className="text-sm text-zinc-400 hover:text-red-400 active:scale-95 transition-all flex items-center gap-1"
                >
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between mb-6">
              ...
              <button 
                ...
                className="p-2 hover:bg-zinc-800 active:scale-90 rounded-full transition-all"
            ...
            <div className="mb-6">
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto pr-2 no-scrollbar">
                {playlists.length === 0 ? (
                  <p className="text-zinc-500 text-sm italic text-center py-4">No playlists available.</p>
                ) : (
                  playlists.map(p => (
                    <button 
                      key={p.id}
                      onClick={() => addToPlaylist(p.id, trackToAdd)}
                      className="w-full text-left px-4 py-3 rounded-xl bg-zinc-800/50 hover:bg-zinc-800 active:scale-[0.98] transition-all flex items-center justify-between group focus:outline-none focus:ring-2 focus:ring-emerald-500 shadow-sm"
                    >
            ...
            <div className="pt-4 border-t border-zinc-800">
              ...
              <form onSubmit={createPlaylist} className="flex gap-2">
                ...
                />
                <button type="submit" className="bg-emerald-500 hover:bg-emerald-400 text-zinc-950 px-4 py-2 rounded-lg font-medium transition-all active:scale-95 shadow-lg">
                  Create
                </button>
              </form>
            </div>
      ...
      {currentTrack && (
        <div className="fixed bottom-0 left-0 right-0 bg-zinc-900/90 backdrop-blur-lg border-t border-white/5 p-3 sm:p-4 z-50">
          <canvas 
            ...
            width={1024} 
            height={64} 
            className="absolute bottom-full left-0 w-full h-16 pointer-events-none opacity-40"
          />
          <div className="max-w-4xl mx-auto flex flex-col sm:flex-row items-center gap-3 sm:gap-6 relative z-10">
            <div className="flex items-center gap-3 w-full sm:w-1/3 min-w-0">
              <img 
                ...
                className="w-10 h-10 sm:w-12 sm:h-12 rounded-md object-cover flex-shrink-0 shadow-lg"
              ...
              <div className="flex sm:hidden items-center gap-1">
                <button 
                  onClick={toggleLyrics}
                  className={`p-2 rounded-full active:scale-90 transition-all ${isLyricsOpen ? 'text-emerald-400 bg-emerald-400/10' : 'text-zinc-400'}`}
                >
            ...
            <div className="w-full sm:flex-1 flex flex-col items-center gap-1 sm:gap-2">
              <div className="flex items-center gap-4 sm:gap-6">
                <button 
                  ...
                  className={`transition-all p-1.5 sm:p-2 rounded-full active:scale-75 ${isShuffle ? 'text-emerald-400 bg-emerald-400/10' : 'text-zinc-400'}`}
                >
                  <Shuffle className="w-4 h-4" />
                </button>
                <button 
                  onClick={playPrevious}
                  className="text-zinc-400 hover:text-zinc-100 active:scale-75 p-1 transition-all"
                >
                  <SkipBack className="w-5 h-5 fill-current" />
                </button>
                <button 
                  onClick={togglePlay}
                  className="w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center bg-white text-black rounded-full hover:scale-105 active:scale-90 transition-all shadow-xl"
                >
                  {isLoadingTrack ? (
                ...
                <button 
                  ...
                  className={`transition-all p-1.5 sm:p-2 rounded-full active:scale-75 ${repeatMode !== 'off' ? 'text-emerald-400 bg-emerald-400/10' : 'text-zinc-400'}`}
                >
            ...
            <div className="w-1/3 hidden sm:flex items-center justify-end gap-4">
              <button 
                onClick={toggleLyrics}
                className={`transition-all p-2 rounded-full active:scale-75 ${isLyricsOpen ? 'text-emerald-400 bg-emerald-400/10' : 'text-zinc-400 hover:text-zinc-100'}`}
              >
              ...
              <div className="flex items-center gap-2">
                ...
                <input
                  ...
                  className="w-16 md:w-24 h-1 bg-zinc-800 rounded-full appearance-none cursor-pointer accent-emerald-500"
                />
              </div>
            </div>
          </div>
